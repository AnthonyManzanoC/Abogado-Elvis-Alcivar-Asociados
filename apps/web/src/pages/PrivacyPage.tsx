...
export function PrivacyPage() {
  ...
  return <article className="policy-page page-section">
    ...
    {settings.map_enabled && <section><h2>Mapa de ubicación</h2><p>{settings.map_load_on_click ? "Google Maps se carga cuando pulsas Ver mapa interactivo." : "La página de contacto incluye un mapa de Google Maps que se carga al acercarte a la sección de ubicación."} Al cargarse, tu navegador se conecta con Google y transmite datos técnicos de conexión. Puedes usar la dirección escrita sin abrir el mapa. Consulta la <a className="text-link" href="https://policies.google.com/privacy?hl=es" target="_blank" rel="noopener noreferrer">política de privacidad de Google</a>.</p></section>}
