...
type Result = { reference: string; startsAt: string; whatsappUrl: string; trackingToken: string };

// `<input type="date">` is a calendar value, not a UTC instant.  Keeping it
// in the visitor's local calendar avoids offering tomorrow as the minimum date
// during the evening in Ecuador/Colombia.
...
export function AppointmentPage() {
    ...
    try {
      const response = await api<Result>("/api/public/appointments", {
        ...
        body: JSON.stringify({
          ...
          practiceArea: area, date, time, modality, reason: form.get("reason"), privacyAccepted: form.get("privacyAccepted") === "on", website: form.get("website")
        })
      });
      setResult(response);
  ...
  if (result) return <section className="booking-success page-section"><div className="success-icon"><Check /></div><span className="eyebrow">Solicitud registrada</span><h1>Tu cita quedó agendada.</h1><p>Conserva este código para cualquier consulta:</p><strong className="reference-code">{result.reference}</strong><p>La notificación quedó en cola de envío. Accede a tu seguimiento para revisar novedades y, si corresponde, adjuntar el comprobante. No compartas tu enlace privado.</p><Link className="button button-gold" to={`/seguimiento#token=${result.trackingToken}`}>Abrir mi seguimiento</Link>{result.whatsappUrl && <a className="button whatsapp-button" href={result.whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle /> Abrir WhatsApp</a>}<small>La reserva no crea por sí sola una relación abogado-cliente.</small></section>;
