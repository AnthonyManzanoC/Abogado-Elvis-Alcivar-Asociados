...
app.use(cors({
  origin(origin, callback) {
    if (isAllowedCorsOrigin(origin, config.frontendUrls)) return callback(null, true);
    callback(new Error("Origen no permitido"));
  },
  // The browser API sends JSON, authorization and the private tracking header.
  // Declare them explicitly so OPTIONS succeeds without opening CORS broadly.
  methods: ["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"],
  allowedHeaders: ["Authorization", "Content-Type", "X-Tracking-Token"],
  maxAge: 600,
